...
def evaluate(args, model, dataloader, device, threshold=0.5):
        ...
        for batch in dataloader:
            ...
            for i, task in enumerate(tasks):
                if task == 0:
                    ...
                    indices_bias.append(i)
                else:
                    labels_stereotype.append(all_labels[i])
                    indices_stereotype.append(i)
            ...
            if logits_bias is not None:
                preds_bias = (logits_bias >= threshold).int().cpu().numpy()
            ...
            if logits_stereo is not None:
                preds_stereo = (logits_stereo >= threshold).int().cpu().numpy()
    ...
    return {
        ...
        'predictions_bias': all_preds_bias,  # Return predictions for bias
        'predictions_stereotype': all_preds_stereotype  # Return predictions for stereotype
    }